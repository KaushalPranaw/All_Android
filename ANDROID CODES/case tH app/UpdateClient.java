...
public class UpdateClient extends AppCompatActivity {
    ...
    public void updateClient(View v)
    {
        ...
        long rw=sb.update(CaseConstant.CLIENT_TBL_NAME,cv,CaseConstant.CLIENT_ID+" =?",args);


        if (rw > 0) { SmsManager sm=SmsManager.getDefault();
            Intent obj=new Intent(this,UpdateClient.class);
            PendingIntent pi=PendingIntent.getActivity(this,1,obj,PendingIntent.FLAG_ONE_SHOT);
            sm.sendTextMessage(phone1,null,"Hello Mr. "+name+ " , Your case is successfully solved.",pi,null);
